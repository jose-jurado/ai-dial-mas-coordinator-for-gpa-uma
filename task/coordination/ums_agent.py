...
class UMSAgentGateway:
    ...
    async def response(
        ...
        if not conversation_id:
            ...
            stage.append_content(f"_Created new UMS conversation: {conversation_id}_\n\n")

        user_message = request.messages[-1].content
        if additional_instructions:
            user_message = f"{user_message}\n\n{additional_instructions}"

        resp_content = await self.__call_ums_agent(
        ...
        return Message(
            role=Role.ASSISTANT,
            content=StrictStr(resp_content),
        )
