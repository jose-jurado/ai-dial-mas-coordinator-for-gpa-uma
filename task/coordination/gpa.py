...
class GPAGateway:
    ...
    async def response(
        ...
        async for chunk in chunks:
            if chunk.choices and len(chunk.choices) > 0:
                ...
                if delta_content := delta.custom_content:
                    print(delta_content)
                    if delta_content.attachments:
                        result_custom_content.attachments.extend(delta_content.attachments)

                    if delta_content.state:
                        result_custom_content.state = delta_content.state

                    if stages := delta_content.dict(exclude_none=True).get("stages"):
                        for stg in stages:
                            idx = stg["index"]
                            if opened_stg := stages.get(idx):
                                if stg_content := stg.get("content"):
                                    ...
                                    StageProcessor.close_stage_safely(stages[idx])
                            else:
                                stages[idx] = StageProcessor.open_stage(choice, stg.get("name"))
    ...
    def __prepare_gpa_messages(self, request: Request, additional_instructions: Optional[str]) -> list[dict[str, Any]]:
            ...
            if msg.role == Role.ASSISTANT:
                if msg.custom_content and msg.custom_content.state:
                    ...
                    if msg_state.get(_IS_GPA):
                        ...
                        resp_messages.append(request.messages[idx-1].model_dump(exclude_none=True))
                        # 2. Copy assistant message
                        copied_msg = deepcopy(msg)
                        copied_msg.custom_content.state = msg_state.get(_GPA_MESSAGES)
                        resp_messages.append(copied_msg.model_dump(exclude_none=True))
        ...
        if additional_instructions:
            ...
            resp_messages.append(last_user_msg.model_dump(exclude_none=True))
